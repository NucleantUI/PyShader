...
//
//  usage: spirv2py <input.spv> [-o output.py] [--entry name] [--rename spirvName=pyName]... [--no-header]
//
//  Interface variables are named after NucleantVulkan's `NucleantShader`
//  layout (`uv`, `frag_coord`, `time`, `resolution`, `mouse`); `--rename`
//  names the ones that layout does not cover.
//
...
do {
    ...
    let shader = try Spirv2PyShader.decompile(data, options: options)
